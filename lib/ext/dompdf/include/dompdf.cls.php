...
class DOMPDF {
   ...
   */
  function render($update_status = false) {  // GCHIESA
    ...
    foreach ($this->_tree->get_frames() as $frame) {
       ...
       if(!($k++ % 100) && ($update_status)) echo ('<sp2html_html2pdf>'.$k.'</sp2html_html2pdf>'); // GCHIESA
    ...
    if($update_status) echo ('<sp2html_html2pdf>END</sp2html_html2pdf>'); // GCHIESA
